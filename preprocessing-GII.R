...
## Gii
# Variable for GII Data from http://hdr.undp.org/en/content/gender-inequality-index-gii
gii <- read.csv("GII_2010.csv")
rawDat$gii <- c()
rawDat$gii[rawDat$city=="Melbourne"]=gii$GII_2010[gii$City=="Melbourne"]
rawDat$gii[rawDat$city=="Minsk"]=gii$GII_2010[gii$City=="Minsk"]
rawDat$gii[rawDat$city=="Chengdu"]=gii$GII_2010[gii$City=="Chengdu"]
rawDat$gii[rawDat$city=="Copenhagen"]=gii$GII_2010[gii$City=="Copenhagen"]
rawDat$gii[rawDat$city=="Bonn"]=gii$GII_2010[gii$City=="Bonn"]
rawDat$gii[rawDat$city=="Athens"]=gii$GII_2010[gii$City=="Athens"]
rawDat$gii[rawDat$city=="Seoul"]=gii$GII_2010[gii$City=="Seoul"]
rawDat$gii[rawDat$city=="Samara"]=gii$GII_2010[gii$City=="Samara"]
rawDat$gii[rawDat$city=="Zurich"]=gii$GII_2010[gii$City=="Zurich"]
rawDat$gii[rawDat$city=="St. Gallen"]=gii$GII_2010[gii$City=="St. Gallen"]
rawDat$gii[rawDat$city=="Istanbul"]=gii$GII_2010[gii$City=="Istanbul"]
rawDat$gii[rawDat$city=="Nottingham"]=gii$GII_2010[gii$City=="Nottingham"]
rawDat$gii[rawDat$city=="Dnipropetrovs'k"]=gii$GII_2010[gii$City=="Dnipropetrovs'k"]
rawDat$gii[rawDat$city=="Boston"]=gii$GII_2010[gii$City=="Boston"]
rawDat$gii[rawDat$city=="Riyadh"]=gii$GII_2010[gii$City=="Riyadh"]
rawDat$gii[rawDat$city=="Muscat"]=gii$GII_2010[gii$City=="Muscat"]
...
for (g in 1:ngroups) {
  ...
  Gii[g] <- mean(redDat$gii[redDat$groupid==group_names[g]&redDat$p=="P-experiment"])  
...
#--- remove groups (from nations) without Gii scores 
Ga_punish <- Ga_punish[,!is.na(Gii)]
Ga_no_punish <- Ga_no_punish[,!is.na(Gii)]

c <- c[,,!is.na(Gii),]
Ga <- Ga[,!is.na(Gii),]
Gc <- Gc[,,!is.na(Gii),]
Nation <- Nation[!is.na(Nation)]
Indiv <- Indiv[!is.na(Gii)]
Civic <- civic[!is.na(Gii)]
GDP <- GDP[!is.na(Gii)]
Trust <- Trust[!is.na(Gii)]
Gii <- Gii[!is.na(Gii)]
...
Gii <- aggregate(Gii~Nation,FUN=mean)[,2]
...
#-------------------  Regress Gii on slope of prefs in CC model ---------------
...
ggplot(postBetaX_omega.df,aes(postBetaX_omega)) + 
  geom_density(colour=4,fill=4,alpha=0.05) + 
  geom_segment(aes(x = CC.betaX_omega.CIL, y = 0, xend = CC.betaX_omega.CIL, yend = 0.55),
               colour=4,size=0.2,linetype="dashed") + 
  geom_segment(aes(x = CC.betaX_omega.HIL, y = 0, xend = CC.betaX_omega.HIL, yend = 0.5),
               colour=4,size=0.2,linetype="dashed") + 
  geom_segment(aes(x = mean(postBetaX_omega), 
                   y = 0, xend = mean(postBetaX_omega), yend = 3.33), 
               colour=4,size=0.2)+
  xlim(-0.5,0.5)+
